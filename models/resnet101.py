...
def fine_tune(lr, epochs, dls=None):
    ...
    plt.savefig("lr_find_resnet101.png")

    # finetune
    learn.fine_tune(epochs, lr)

    # save model
    learn.export('ckpts/export_res101.pkl')


def predict(test_files):
    print(f"[+] Predicting {len(test_files)} images...")
    learn = load_learner('ckpts/export_res101.pkl')
    ...
    df = pd.DataFrame(val)
    print(df.head())
    df.to_csv('submission_res101.csv', index=False)
