...
def fine_tune(lr, epochs, dls=None):
    ...
    print("[+] Fine tuning...")
    # create learner
    learn = vision_learner(dls, "vgg19", metrics=error_rate)
    ...
    plt.savefig("lr_find_vgg19.png")

    # finetune
    learn.fine_tune(epochs, lr)

    # save model
    learn.export('ckpts/export_vgg19.pkl')


def predict(test_files):
    print(f"[+] Predicting {len(test_files)} images...")
    learn = load_learner('ckpts/export_vgg19.pkl')
    ...
    df = pd.DataFrame(val)
    print(df.head())
    df.to_csv('submission_vgg19.csv', index=False)
