...
def fine_tune(lr, epochs, dls=None):
    ...
    print("[+] Fine tuning...")
    # create learner
    learn = vision_learner(dls, "vit_base_r50_s16_224", metrics=error_rate)
    ...
    plt.savefig("lr_find_vit_r50.png")

    # finetune
    learn.fine_tune(epochs, lr)

    # save model
    learn.export('ckpts/export_vit_base_r50_s16_224.pkl')


def predict(test_files):
    print(f"[+] Predicting {len(test_files)} images...")
    learn = load_learner('ckpts/export_vit_base_r50_s16_224.pkl')
    ...
    df = pd.DataFrame(val)
    print(df.head())
    df.to_csv('submission_vit_r50.csv', index=False)
